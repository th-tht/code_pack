...
class model(Unpack_data): 
    ...
    def heat_load_solve(self):
        ...
        for st in self.heat_match.keys():
            self.heat_match[st].sort()
        
        if self.heat_match != self.matches:
            self.matches = self.heat_match.copy()
            ...
            self.loadfunc = toplogical_sort(self.matches, enthepy)
                                
    def update_and_solve(self, hu):
        
        for key in self.q_idx.keys():
            
            if type(self.loadfunc[key]) == Numeric:
                self.q[key] = self.loadfunc[key].value(hu) 
            else:
                self.q[key] = self.loadfunc[key]
